...
export function Switch({ checked, onCheckedChange, disabled }: {
  ...
  return <RadixSwitch.Root checked={checked} onCheckedChange={onCheckedChange} disabled={disabled} className="relative h-5 w-9 rounded-full border border-app-border bg-app-muted outline-none transition-colors data-[state=checked]:border-app-primary data-[state=checked]:bg-app-primary disabled:opacity-50">
...
export function Checkbox({ checked, onCheckedChange, label, disabled }: {
  ...
  return <RadixCheckbox.Root aria-label={label} checked={checked} disabled={disabled} onCheckedChange={(value) => onCheckedChange(value === true)} className="grid size-4 place-items-center rounded border border-app-border bg-app-muted text-[11px] text-black outline-none data-[state=checked]:border-app-primary data-[state=checked]:bg-app-primary disabled:pointer-events-none disabled:opacity-40">
...
export function Select({ value, onValueChange, options, placeholder, ariaLabel = "选择选项" }: {
  ...
  return <RadixSelect.Root value={value} onValueChange={onValueChange}>
    <RadixSelect.Trigger aria-label={ariaLabel} className="inline-flex h-8 min-w-28 items-center justify-between gap-2 rounded-md border border-app-border bg-app-muted px-2 text-xs outline-none focus:ring-0">
  ...
  return <RadixDialog.Root open={open} onOpenChange={onOpenChange}>
    <RadixDialog.Portal>
      ...
      <RadixDialog.Content className={cn("fixed left-1/2 top-1/2 z-50 max-h-[88vh] w-[min(92vw,680px)] -translate-x-1/2 -translate-y-1/2 overflow-auto overscroll-contain rounded-xl border border-app-border bg-app-surface p-5 shadow-2xl outline-none", contentClassName)}>
        ...
        {children}<RadixDialog.Close className="absolute right-3 top-3 grid size-7 place-items-center rounded-md text-app-subtle outline-none hover:bg-app-hover hover:text-app-foreground" aria-label="关闭">
          <CloseIcon />
        </RadixDialog.Close>
      </RadixDialog.Content>
    </RadixDialog.Portal>
